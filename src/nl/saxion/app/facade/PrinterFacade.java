...
public class PrinterFacade {
	...
	private void readPrintsFromFile(String filename) throws org.json.simple.parser.ParseException {
		...
		try (FileReader reader = new FileReader(URLDecoder.decode(printResource.getPath(), StandardCharsets.UTF_8))) {
			...
			for (Object p : prints) {
				...
				ArrayList<Double> filamentLength = new ArrayList<Double>();
	...
	private void readPrintersFromFile(String filename) throws org.json.simple.parser.ParseException {
		JSONParser jsonParser = new JSONParser();
		if(filename.length() == 0) {
	...
	private void readSpoolsFromFile(String filename) throws org.json.simple.parser.ParseException {
		JSONParser jsonParser = new JSONParser();
		if(filename.length() == 0) {
